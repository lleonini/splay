...
class SplaydProtocol
	...
	@@sleep_time = Config::SPSleepTime
	@@ping_interval = Config::SPPingInterval
	@@socket_timeout = Config::SPSocketTimeout
	@@logd_ip = Config::LogdIP
	@@logd_port = Config::LogdPort
	@@log_max_size = Config::LogMaxSize
	@@num_logd = Config::NumLogd
	@@localize = Config::Localize
	@@nat_gateway_ip = Config::NATGatewayIP
...
$log.info("BENCH #{@splayd.id} #{action['job_id']} - ACTION #{action['command']} TIME: #{reply_time}")
...
class Splayd
	...
	@@unseen_timeout = 3600
	@@auto_add = Config::AutoAddSplayds
	...
	def ip_check ip
		if ip == "127.0.0.1" or not $db.select_one "SELECT * FROM splayds WHERE
				ip='#{ip}' AND
				`key`!='#{@row['key']}' AND
